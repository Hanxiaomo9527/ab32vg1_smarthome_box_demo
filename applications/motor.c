...
#define MOTOR_PIN           rt_pin_get("PA.6")
...
rt_uint32_t period = 100;   /* 周期为10ms，单位为纳秒ns */
rt_uint32_t pulse = 0;           /* PWM脉冲宽度值，单位为纳秒ns */
#endif
void set_motor_value(rt_uint32_t pwm_value)
{
    rt_pin_write(MOTOR_PIN, pwm_value);    // 拉高LED引脚
#if 0
    rt_uint32_t t = 0;
...
//    rt_pin_write(MOTOR_PIN, PIN_HIGH);    // 拉高LED引脚
    ...
    rt_pwm_set(motor_pwm_dev, PWM_DEV_CHANNEL, period, 100);// 设置PWM周期和脉冲宽度默认值
